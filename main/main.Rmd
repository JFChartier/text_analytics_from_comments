...
vector_of_categorieAppel = myTextVectorizer(tokenizedCorpus = (tokens_from_cat), my_doc_count_min = 0, my_doc_proportion_max = 1) #%>% scale(., center = T, scale = F)
...
rownames(reduced_categories_space) = unique(categorie_appel) #vector_of_categorieAppel@Dimnames$docs
...
unique_word_preprocessed = text_preprocessing(text_in_claims = unique_word_in_comments, myFrenchStopwords = myFrenchStopwords, my_doc_count_min = 0, my_doc_proportion_max = 1, to_stemm = T, language = "fr", spelling_checker = F, charNgram = 4) #sapply(unique_word_in_comments, function(x) paste("dummy", x, "dummy", sep = " ")) %>% 
...
similarity_between_cat_word=text2vec::sim2(x = as.matrix(reduced_categories_space), y = as.matrix(reduced_word_space), method = "cosine", norm = "none") #%>% 
View(t(similarity_between_cat_word), title = "spec6")
#set_rownames(dat)
...
j= rownames(reduced_categories_space)=="brio" #"rap / reep" #"reee" #"rpe" #"brio"   #"rap / reep"
...
DT::datatable(data = data.frame(comments_from_agents[i,c("Categorie_dappel", "Commentaires_de_lagent")], similarity=similarity_between_comments_cat[i, j]), rownames = T, filter="none", selection="none", options = list(pageLength = 20, scrollX=T), caption=paste("Commentaires les plus similaire au code: ", rownames(reduced_categories_space)[j], ""))
```
